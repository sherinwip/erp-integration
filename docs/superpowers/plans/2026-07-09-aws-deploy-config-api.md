...
- ALB target group health check probes `/docs` (alb.tf) rather than the app's purpose-built `/health` endpoint (`erp-config-api/app/main.py`). Both are DB-independent so behavior is equivalent, but `/health` is lighter (no Swagger UI render) and more semantically correct. Left as-is to avoid an additional live `terraform apply` for a non-functional change; switching `health_check.path` to `/health` is a safe one-line follow-up whenever the target group is next touched.
- Docker images must be built for `linux/arm64` (matches `db.t4g.micro` Graviton and the ECS task defs' `runtime_platform { cpu_architecture = "ARM64" }`, added during Task 7 execution after "exec format error" surfaced on X86_64 task defs built from an Apple Silicon host). Building on an amd64 host, or without an arm64-capable builder, will produce an image ECS cannot run.
- **RESOLVED (commit 839150b): Liquibase migration previously stopped at changeset 015/36.** `changelog/changes/015-demo-new-object-step17.xml` (and half of `018-demo-object-target-showcase-step17`) inserted `field_mapping` rows referencing `step_pk=17`, but no changeset in this repo's changelog ever created a `step` row with that pk (its owning client, "sawan-award-to-oracle-contract-demo-v1", was never seeded either). This silently blocked changeset 016 (`field_mapping.is_object_target` column) and 017 (`transform_payload()` update using it) — surfaced live as `POST /api/v1/field-mappings/bulk` returning 500 `column field_mapping.is_object_target does not exist`. Fix: excluded the two step_pk=17 changesets (015 entirely via a comment in `db.changelog-master.xml`; 018's step17-only `changeSet`, keeping its step_pk=3 half) rather than inventing fixture client/target/step data with no real source. Verified locally against a fresh Postgres container: 34/34 remaining changesets apply cleanly, `is_object_target` column confirmed present. **Still needs**: rebuild+push the `liquibase-runner` image with this fix and re-run the one-off ECS Liquibase task against RDS (RDS itself still only has 31/36 applied as of this note).
