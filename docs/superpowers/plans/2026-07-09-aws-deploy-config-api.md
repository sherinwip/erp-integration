...
- **Known gap: Liquibase migration currently stops at changeset 015/36.** `changelog/changes/015-demo-new-object-step17.xml` (and half of `018-demo-object-target-showcase-step17`) insert `field_mapping` rows referencing `step_pk=17`, but no changeset in this repo's changelog ever creates a `step` row with that pk — confirmed by running the migration against a brand-new RDS instance, not stale local data. The one-off Liquibase ECS task applies changesets 1-31 successfully (full schema, all core seed data, most demo data) and fails cleanly on changeset 015; changesets 016-018 never run as a result. `erp-config-api` does not depend on this demo dataset for correctness — confirmed live via `/api/v1/clients` and `/api/v1/steps` returning real data through the deployed ALB. Follow-up (not done): add a changeset that seeds the missing `step_pk=17` row (or renumber to an existing step), then re-run `aws ecs run-task` with the Liquibase task definition to pick up changesets 015-018.
